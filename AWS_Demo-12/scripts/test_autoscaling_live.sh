#!/usr/bin/env bash
# API scales on ALB/CPU; worker scales on Celery QueueDepth.
set -euo pipefail
...
ROOT_PROOF="${PROOF_DIR:-${SCRIPT_DIR}/../proof/autoscaling-live-$(date +%Y%m%d-%H%M%S)}"
...
echo "=== Autoscaling live test ===" | tee "${PROOF_DIR}/README.txt"
...
echo "" | tee -a "${PROOF_DIR}/README.txt"
echo "=== Test 1: Worker scale-out (QueueDepth custom metric) ===" | tee "${PROOF_DIR}/02-worker-test.txt"
...
aws cloudwatch put-metric-data --region "${AWS_REGION}" \
  ...
  --metric-data "MetricName=QueueDepth,Value=25,Unit=Count" \
  | tee -a "${PROOF_DIR}/02-worker-test.txt"
...
(
  while true; do
    ...
    aws cloudwatch put-metric-data --region "${AWS_REGION}" \
      --namespace "${METRIC_NAMESPACE}" \
      --metric-data "MetricName=QueueDepth,Value=25,Unit=Count" >/dev/null
  done
) &
metric_pid=$!
...
echo "" | tee -a "${PROOF_DIR}/README.txt"
echo "=== Test 2: API scale-out (ALB requests + CPU via sustained load) ===" | tee "${ROOT_PROOF}/03-api-test.txt"
...
echo "" | tee -a "${ROOT_PROOF}/README.txt"
echo "=== Scale-in signals (restore baseline) ===" | tee "${ROOT_PROOF}/04-scale-in.txt"
aws cloudwatch put-metric-data --region "${AWS_REGION}" \
  --namespace "${METRIC_NAMESPACE}" \
  --metric-data "MetricName=QueueDepth,Value=0,Unit=Count" | tee -a "${ROOT_PROOF}/04-scale-in.txt"
echo "Worker/API will scale in after cooldown (2–3 min). Current counts:" | tee -a "${ROOT_PROOF}/04-scale-in.txt"
get_counts | tee -a "${ROOT_PROOF}/04-scale-in.txt"

echo "" | tee "${ROOT_PROOF}/SUMMARY.txt"
{
  echo "Autoscaling live test summary — $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  echo "Worker scale-out (QueueDepth metric): $([[ ${worker_pass:-0} -eq 1 ]] && echo PASS || echo FAIL)"
  echo "API scale-out (load test):          $([[ ${api_pass:-0} -eq 1 ]] && echo PASS || echo FAIL)"
